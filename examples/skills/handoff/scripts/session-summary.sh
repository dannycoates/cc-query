...
SELECT strftime(timestamp, '%m-%d %H:%M:%S') as timestamp, id, type,
  ...
  SELECT tu.timestamp, tu.tool_id as id, lower(tu.tool_name) as type,
         CASE
           ...
           WHEN tu.tool_name = 'Bash' THEN
             ...
             CASE WHEN length(tu.tool_input->>'command') > 100 THEN left(tu.tool_input->>'command', 100) || '...' ELSE tu.tool_input->>'command' END ||
             ' → ' ||
           ...
           WHEN tu.tool_name = 'Write' THEN
             ...
             CASE WHEN tr.is_error THEN '[ERR] ' || left(tr.result_content, 200)
           ...
           WHEN tu.tool_name = 'Edit' THEN
             tu.tool_input->>'file_path' ||
             CASE WHEN tr.is_error THEN ' | [ERR] ' || left(tr.result_content, 200)
                  ELSE ' | old:' || COALESCE(left(tu.tool_input->>'old_string', 100), '') ||
                       ' | new:' || COALESCE(left(tu.tool_input->>'new_string', 100), '') END
           -- Medium: Read
           ...
           WHEN tu.tool_name = 'TodoWrite' THEN
             CASE WHEN length(tu.tool_input->>'todos') > 200 THEN left(tu.tool_input->>'todos', 200) || '...'
